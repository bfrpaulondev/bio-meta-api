...
const { openaiService } = require("../../shared/services/openai");
...
async function aiRoutes(fastify) {
  ...
  fastify.post("/insights", {
    ...
    try {
      const insight = await openaiService.generateFitnessInsight(request.body);
  ...
  fastify.post("/workout-recommendation", {
    ...
    try {
      const recommendation = await openaiService.generateWorkoutRecommendation(request.body);
  ...
  fastify.post("/nutrition-advice", {
    ...
    try {
      const advice = await openaiService.generateNutritionAdvice(request.body);
